...
if(!isset($_SESSION['user_id'])){
    if(isAjaxRequest()){
        sendErrorResponse("Please login to continue", [], 401);
    }
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $verification_method = isset($_POST['method']) ? $_POST['method'] : 'AI Image Analysis';
    ...
    if(empty($barcode) && empty($batch_number)){
        if(isAjaxRequest()){
            sendErrorResponse("Please enter either a barcode or batch number");
        }
    ...
    if($medicine){
        ...
        if(isAjaxRequest()){
            sendSuccessResponse("Verification completed successfully!", [
                'verification_id' => $verification_id,
                'verification_result' => $verification_result,
                'confidence_score' => $confidence_score,
                'medicine' => $medicine,
                'ai_analysis' => $aiReport,
                'redirect' => '../Views/verification_result.php?id=' . $verification_id
            ]);
        }
        ...
        if(isAjaxRequest()){
            sendSuccessResponse("Medicine not found in database. AI analysis completed.", [
                'verification_id' => $verification_id,
